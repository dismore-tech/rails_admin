        ...
        class BelongsToAssociation < RailsAdmin::Config::Fields::SingularAssociation
          ...
          def selected_id
            if association.foreign_key.is_a?(Array)
              serialize_id(association.foreign_key.map { |attribute| bindings[:object].safe_send(attribute) })
            else
          ...
          def parse_input(params)
            ...
            association.foreign_key.zip(associated_model_config.abstract_model.parse_id(params.delete(method_name))).each do |key, value|
              params[key] = value
            end
          end
        end

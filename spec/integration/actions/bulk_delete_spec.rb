...
RSpec.describe 'BulkDelete action', type: :request do
  ...
  context 'with composite_primary_keys', composite_primary_keys: true do
    ...
    it 'deletes selected records' do
      delete(bulk_delete_path(bulk_action: 'bulk_delete', model_name: 'fanship', bulk_ids: fanships[0..1].map { |fanship| RailsAdmin::Support::CompositeKeySerializer.serialize(fanship.id) }))
